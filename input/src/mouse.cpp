...
bool Ps2Mouse::wait_read() {
	...
	for (int i = 0; i < 100000; ++i) {
		if (inb(PS2_STATUS) & PS2_STATUS_OUTPUT) return true;
	}
#endif
	return false;
}
...
#if defined(__x86_64__)
	for (int i = 0; i < 100000; ++i) {
		if ((inb(PS2_STATUS) & PS2_STATUS_INPUT) == 0) return true;
	}
#endif
	return false;
}
...
bool Ps2Mouse::initialize() {
	...
	if (!write_command(0xA8)) return false;
	// Enable IRQ12 (read controller command byte, set bit1)
	if (!write_command(0x20)) return false; // Read command byte
	uint8_t cmd = 0;
	if (!read_data(cmd)) return false;
	...
	if (!write_command(0xD4)) return false;
	if (!write_device(0xF4)) return false; // Enable data reporting
	// Expect ACK 0xFA
	uint8_t ack = 0;
	if (!read_data(ack)) return false;
	return ack == 0xFA;
#else
	return false;
#endif
...
bool Ps2Mouse::poll_packet(int8_t &dx, int8_t &dy, bool &left, bool &right, bool &middle) {
#if defined(__x86_64__)
	// Attempt to read a 3-byte packet
	if (!wait_read()) return false;
	uint8_t b1 = inb(PS2_DATA);
	if ((b1 & 0x08) == 0) return false; // sync bit not set
	if (!wait_read()) return false;
	...
	return true;
#else
	(void)dx; (void)dy; (void)left; (void)right; (void)middle;
	return false;
#endif
}
