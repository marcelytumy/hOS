...
void Cursor::set_position(uint32_t x, uint32_t y, uint32_t screen_w,
                          uint32_t screen_h) {
  if (x >= screen_w)
    x = screen_w - 1;
  if (y >= screen_h)
...
void Cursor::move_by(int32_t dx, int32_t dy, uint32_t screen_w,
                     uint32_t screen_h) {
  ...
  if (ny < 0)
    ny = 0;
  if (nx >= static_cast<int64_t>(screen_w))
    nx = screen_w - 1;
  if (ny >= static_cast<int64_t>(screen_h))
    ny = screen_h - 1;
...
void Cursor::invalidate() { has_saved = false; }
